...
{
	uint16_t* instruction = reinterpret_cast<uint16_t*>((void*)info.rip);

	if (*instruction == 0x0B0F) // ud2
	{
		KDEBUG_RICHPANIC("Invalid opcode.",
			"#UD",
			false,
			"Op code: 0x%x (UD2) generated by compiler for undefined behaviors.\nInspect the code!\n",
			(uint32_t)(*instruction));

	}
	else
	{
		KDEBUG_RICHPANIC("Invalid opcode.", "#UD", false, "Op code: 0x%x", (uint32_t)(*instruction));
	}
	return ERROR_SUCCESS;
}
