...
void debug_write_format_a(const char* fmt, va_list ap)
{
	...
	for (i = 0; (c = char_data(fmt[i])) != 0; i++)
	{
		...
		case 'f':
		{
			...
			put_str("%f flags_ is disabled because va_arg(ap, double) always return wrong value");
...
void debug_write_format(const char* fmt, ...)
{

	va_list ap;
	va_start(ap, fmt);

	debug_write_format_a(fmt, ap);
